        ...
        <div class="card">
            <div class="d-flex flex-wrap flex-stack my-5 mx-8">
                ...
                <div class="d-flex my-2">
                    @if($puedeCrear)
                    <button type="button" class="btn btn-primary px-4 px-sm-6" x-data="{ cargando: false }" @click="cargando = true; $dispatch('abrirModalRol')" @cargando.window="cargando = false" :disabled="cargando">
                        ...
                        <table class="table align-middle table-row-dashed fs-6 gy-5 dataTable no-footer">
                            <thead>
                                <tr class="text-start text-muted fw-bold text-uppercase gs-0">
                                    ...
                                    <th class="min-w-125px">NOMBRE ROL</th>
                            ...
                            <tbody class="text-gray-600" wire:loading.class="opacity-25" wire:target="buscar, gotoPage, previousPage, nextPage">
                                ...
                                <tr wire:key="{{ $item->id_rol }}">
                                    ...
                                    <td>
                                        @if ($item->estado_rol === \App\Enums\EstadoEnum::HABILITADO->value)
                                    ...
                                    <td class="text-center">
                                        @if( $puedeModificar || $puedeEliminar || $puedeCambiarEstado || $puedeAsignarAcceso)
                                            ...
                                            <div class="dropdown-menu dropdown-menu-end menu menu-sub menu-sub-dropdown
                                                        ...
                                                        fw-semibold fs-6 w-125px py-4" data-kt-menu="true" x-data="{ cargando_opciones: false }" onclick="event.stopPropagation();">
                                                ...
                                                <div class="menu-item px-3" x-data="{ cargando: false }" @cargando.window="cargando = false; cargando_opciones = false" :class="{ 'item-disabled': cargando_opciones }">
                                                    <a class="menu-link px-3 d-flex align-items-center gap-2" @click="
                                                                cargando = true;
                                                                cargando_opciones = true;
                                                                $dispatch('abrirModalRol', { idRol: {{ $item->id_rol }} });
                                                            " :class="{ 'active': cargando }" onclick="event.stopPropagation();" wire:ignore>
                                                        <template x-if="!cargando">
                                                            <span>Modificar</span>
                                                        </template>
                                                ...
                                                @if($puedeAsignarAcceso)
                                                <div class="menu-item px-3" x-data="{ cargando: false }" @cargando.window="cargando = false; cargando_opciones = false" :class="{ 'item-disabled': cargando_opciones }">
                                                    <a class="menu-link px-3 d-flex align-items-center gap-2" x-data="{ cargando: false }" @click="
                                                                cargando = true;
                                                                window.location.href='{{ route('seguridad.roles.acceso.index', ['rol' => encriptar($item->id_rol)]) }}';
                                                            " :class="{ 'active': cargando }" onclick="event.stopPropagation();" wire:ignore>
                                                        <template x-if="!cargando">
                                                ...
                                                <div class="menu-item px-3">
                                                    <a class="menu-link px-3" x-data="{ cargando: false }" @click="
                                                                cargando = true;
                                                                $dispatch('abrirModalEstadoRol', { idRol: {{ $item->id_rol }} });
                                                            " @cargando.window="cargando = false" :class="{ 'active': cargando }" onclick="event.stopPropagation();">
                                                        Estado
                                                        <template x-if="cargando">
                                                            <span>
                                                                <x-spinner class="ms-2" style="width: 20px; height: 20px;" />
                                                ...
                                                @if($puedeEliminar)
                                                <div class="menu-item px-3">
                                                    <a class="menu-link px-3 text-danger" x-data="{ cargando: false }" @click="
                                                                cargando = true;
                                                                $dispatch('abrirModalEliminarRol', { idRol: {{ $item->id_rol }} });
                                                            " @cargando.window="cargando = false" :class="{ 'active': cargando }" onclick="event.stopPropagation();">
                                                        Eliminar
                                                        <template x-if="cargando">
                                                            <span>
                        ...
                        <div>
                            @if ($this->roles->hasPages())
                                <div class="d-flex justify-content-between">
                                    <div class="d-flex align-items-center">Mostrando {{ $this->roles->firstItem() }} - {{ $this->roles->lastItem() }} de {{ $this->roles->total() }} registros</div>
                                    <div class="pagination pagination-lg">{{ $this->roles->links() }}</div>
                                </div>
                            @else
                                <div class="d-flex justify-content-between py-2">
                                    <div class="d-flex align-items-center text-muted
                                    ">Mostrando {{ $this->roles->firstItem() }} - {{ $this->roles->lastItem() }} de {{ $this->roles->total() }} registros</div>
                                </div>
                            @endif
                        </div>
                    </div>
